...
class Main {
    ...
    async run(outputSink, inputForScalarF) {
        const output = new Output(outputSink);
        try {
            // header - (label 1)
            output.printExpr("WAVES");output.println();
            // ask for number of waves - (label 2)
            output.printExpr("TYPE IN HOW MANY WAVES TO DRAW AS A NUMBER BETWEEN 1 AND 4?");
            output.println();
            const numberOfWaves = await inputForScalarF.fetch();
            output.println();
            // run the main logic
            const result = this.makeWaves(numberOfWaves);
            // output the result
            output.printExpr(result);
            output.println();
        } catch (e) {
    ...
    makeWaves(numberOfWaves) {
        ...
        while (true) {
            ...
            switch (label) {
                ...
                case 12:
                    label = 13;
                    result += this.mid(stringW, scalarJ, scalarS);
                    break;
                // 13NEXTJ
                case 13:
                    label = 14;
